...
export function EvidencePanel() {
  ...
  return (
    <div className="relative mx-auto w-full max-w-xl lg:max-w-none">
      <div aria-hidden className="orb -top-20 -right-12 size-[28rem]" style={{ "--orb": "rgb(155 128 255 / 0.5)" } as CSSProperties} />
      <div aria-hidden className="orb -bottom-24 -left-16 size-80" style={{ "--orb": "rgb(32 186 158 / 0.28)" } as CSSProperties} />

      <figure
        className="glass rise relative overflow-hidden rounded-[1.75rem] p-4 sm:p-6"
      ...
      >
        ...
        <ul aria-label="Connected systems" className="mt-4 flex flex-wrap gap-1.5">
          {heroDemo.connected.map((s) => (
        ...
        <div className="panel ledger mt-5 rounded-2xl p-4 sm:p-5">
            ...
            {heroDemo.findings.map((f) => (
              <li key={f.label} className="flex flex-col gap-0.5 px-3.5 py-2.5 text-sm sm:flex-row sm:items-center sm:justify-between sm:gap-3">
                <span className="flex items-center gap-2.5">
                  <span
                    aria-hidden
                    className={cn("size-2 rounded-full", f.status === "risk" ? "bg-[#ff254e]" : "bg-[#f5a524]")}
                  />
          ...
          <dl className="mt-4 grid gap-x-3 gap-y-1 font-mono text-[0.6875rem] leading-relaxed text-muted sm:grid-cols-[auto_1fr]">
            <dt className="uppercase tracking-[0.12em]">Sources</dt>
            <dd>{heroDemo.sources.join("  ·  ")}</dd>
            <dt className="mt-1 uppercase tracking-[0.12em] sm:mt-0">Next step</dt>
            <dd className="text-ink-soft">{heroDemo.action}</dd>
          </dl>
        </div>

        <p className="mt-3 text-right font-mono text-[0.6875rem] uppercase tracking-[0.14em] text-muted">{heroDemo.label}</p>
      </figure>
      ...
      {heroDemo.chips.map((chip, i) => {
        ...
        return (
          <div
            ...
            aria-hidden
            className={cn(
              "glass rise absolute hidden max-w-[16rem] items-center gap-2.5 rounded-2xl py-2 pr-3.5 pl-2 lg:flex",
              i === 0 ? "-top-7 right-10" : "-bottom-8 -left-10",
            )}
